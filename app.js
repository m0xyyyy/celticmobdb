...
function renderMapDots(spawns,zone){const box=$('#mapDots');if(!box)return;box.innerHTML='';if(!zone)return;const w=zone.maxX-zone.minX||1,h=zone.maxZ-zone.minZ||1;const pad=0.08;
spawns.forEach(s=>{const d=document.createElement('div');d.className='spawn-dot';let nx=(s.x-zone.minX)/w;let nz=(s.z-zone.minZ)/h;nx=pad+nx*(1-2*pad);nz=pad+nz*(1-2*pad);const px=nx*100;const py=(1-nz)*100;
const firstMobId=Array.isArray(s.mobs)&&s.mobs.length?s.mobs[0].mobId:null;const mob=getMob(firstMobId);const mobName=mob?.name||'Unknown mob';
d.dataset.mobName=mobName;d.setAttribute('aria-label',mobName);d.style.left=px+'%';d.style.top=py+'%';box.appendChild(d)})}
...
/* --- Spawn mob-name tooltip: presentation only --- */
(() => {
  ...
  const move=(x,y)=>{
    ...
    tip.style.left=`${Math.max(12,Math.min(innerWidth-r.width-12,x+14))}px`;
    tip.style.top=`${Math.max(12,Math.min(innerHeight-r.height-12,y-r.height-12))}px`;
  };
  const hide=()=>{tip.classList.remove('show');active=null;};
  document.addEventListener('pointerover',e=>{
    const el=e.target.closest('.spawn-dot');
    if(!el)return;
    tip.textContent=el.dataset.mobName||'Unknown mob';
    tip.classList.add('show');active=el;move(e.clientX,e.clientY);
  });
  document.addEventListener('pointermove',e=>{if(active)move(e.clientX,e.clientY)},{passive:true});
