...
function renderMapDots(spawns,zone){const box=$('#mapDots');if(!box)return;box.innerHTML='';if(!zone)return;const w=zone.maxX-zone.minX||1,h=zone.maxZ-zone.minZ||1;const pad=0.08;spawns.forEach(s=>{const d=document.createElement('div');d.className='spawn-dot';let nx=(s.x-zone.minX)/w;let nz=(s.z-zone.minZ)/h;nx=pad+nx*(1-2*pad);nz=pad+nz*(1-2*pad);const px=nx*100;const py=(1-nz)*100;const mobIds=Array.isArray(s.mobs)?s.mobs.map(x=>x.mobId):[];const mobName=mobIds.map(id=>getMob(id)?.name).find(Boolean)||'Unknown mob';d.dataset.mobName=mobName;d.title=mobName;d.setAttribute('aria-label',mobName);d.style.left=px+'%';d.style.top=py+'%';box.appendChild(d)})})();
...
(() => {
  ...
  const show = (el, x, y) => {
    ...
    tip.style.left = `${Math.max(12, Math.min(window.innerWidth - rect.width - 12, x + 14))}px`;
    tip.style.top = `${Math.max(12, Math.min(window.innerHeight - rect.height - 12, y - rect.height - 12))}px`;
  };
  ...
  document.addEventListener('pointermove', e => {
    if (!active) return;
    const rect = tip.getBoundingClientRect();
    tip.style.left = `${Math.max(12, Math.min(window.innerWidth - rect.width - 12, e.clientX + 14))}px`;
    tip.style.top = `${Math.max(12, Math.min(window.innerHeight - rect.height - 12, e.clientY - rect.height - 12))}px`;
  }, {passive:true});
