...
(() => {
  ...
  const show = (el, x, y) => {
    ...
    tip.style.left = `${Math.max(12, Math.min(window.innerWidth-r.width-12, x+14))}px`;
    tip.style.top = `${Math.max(12, Math.min(window.innerHeight-r.height-12, y-r.height-12))}px`;
  };
  ...
  document.addEventListener('pointermove', e => {
    if (!active) return;
    const r = tip.getBoundingClientRect();
    tip.style.left = `${Math.max(12, Math.min(window.innerWidth-r.width-12, e.clientX+14))}px`;
    tip.style.top = `${Math.max(12, Math.min(window.innerHeight-r.height-12, e.clientY-r.height-12))}px`;
  }, {passive:true});
